...
const COST_SCALE_MULTIPLIER: f64 = 1000.0;
...
fn scaled_cost(value: f64) -> f64 {
    value * COST_SCALE_MULTIPLIER
}

/// Material cost [USD]: mass × cost_per_kg, summed across zones, scaled to project cost basis.
pub fn material_cost_usd(config: &VesselConfig) -> f64 {
    scaled_cost(raw_material_cost_usd(config))
}

fn raw_material_cost_usd(config: &VesselConfig) -> f64 {
...
pub fn weld_cost_usd(config: &VesselConfig) -> f64 {
    scaled_cost(raw_weld_cost_usd(config))
}

fn raw_weld_cost_usd(config: &VesselConfig) -> f64 {
    let h = &config.hull;
...
pub fn seal_cost_usd(config: &VesselConfig) -> f64 {
    scaled_cost(raw_seal_cost_usd(config))
}

fn raw_seal_cost_usd(config: &VesselConfig) -> f64 {
    let h = &config.hull;
...
pub fn fuel_capacity_cost_usd(config: &VesselConfig) -> f64 {
    scaled_cost(raw_fuel_capacity_cost_usd(config))
}

fn raw_fuel_capacity_cost_usd(config: &VesselConfig) -> f64 {
